...
export default function NavMenu({
  ...
  const { user, isAuthenticated } = useAuth(); // add this import
  ...
  const menuItems = [
    ...
    },
    {
      ...
      subItems: [
        ...
        {
          label: "SmartWatches",
          route: "/smartwatches?deviceType=refurbished",
        },
    ...
    },
    {
      ...
      subItems: [
        { label: "Find New Mobiles", route: "/phones?deviceType=new" },
        { label: "Find New Laptops", route: "/laptops?deviceType=new" },
        { label: "Find New Tablets", route: "/tablets?deviceType=new" },
        {
          label: "Find New SmartWatches",
          route: "/smartwatches?deviceType=new",
        },
        { label: "Find New Television", route: "/televisions?deviceType=new" },
      ],
    },
    {
      title: "Buy Laptop",
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-center gap-1 md:gap-3 lg:gap-8 xl:gap-24">
            {menuItems.map((item, idx) => (
              <div key={idx} className="relative py-3 min-w-0">
                <button
                  onMouseEnter={() => handleMenuClick(idx)}
                  className={`flex items-center gap-1 px-1 md:px-2 lg:px-3 py-1.5 text-xs md:text-sm font-medium cursor-pointer whitespace-nowrap border-b-2 transition-all duration-150 ${
                    openMenu === idx
                          ...
                          <div key={si} className="relative">
                            <button
                              ...
                              className={`w-full flex items-center justify-between px-4 py-2.5 rounded-2xl text-sm font-medium cursor-pointer transition-colors duration-100 ${
                                isSubOpen
                                  ? "bg-teal-50 text-[#0077b6]"
                                  : "text-gray-700 hover:bg-[#caf0f8] hover:text-[#0077b6]"
                              }`}
                            >
                              <span>{sub.label}</span>
                              {sub.subs?.length > 0 && (
      ...
      <div
        className={`fixed top-0 right-0 h-full w-[300px] bg-white z-50 shadow-2xl transform transition-transform duration-300 md:hidden overflow-y-auto ${
