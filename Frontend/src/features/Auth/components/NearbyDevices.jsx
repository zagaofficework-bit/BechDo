...
async function fetchNearbyProducts({
  ...
  if (!res.ok)
    throw new Error(data.message || "Failed to fetch nearby products");
  return data;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) =>
        reject(
          new Error(
            ["", "Permission denied", "Unavailable", "Timed out"][err.code] ||
              "Unknown error",
          ),
        ),
...
const NearbyCard = memo(({ product, onClick }) => {
  ...
  useEffect(() => {
    return () => {
      if (product.images?.[0]?.startsWith("blob:")) {
        URL.revokeObjectURL(product.images[0]);
      }
    };
  }, []);
      ...
      >
        {product.images?.[0] ? (
          <img
            ...
            onError={(e) => {
              e.target.src =
                "https://placehold.co/240x180/e4eef7/0077b6?text=📱";
            }}
          />
        ) : (
      ...
      <div style={{ padding: "14px 14px 16px" }}>
        ...
        {product.address?.city && (
          ...
          >
            ...
            {product.address.city}
            {product.address.state ? `, ${product.address.state}` : ""}
          </p>
        )}
        ...
        {product.listedBy?.firstname ? (
          ...
          <p style={{ fontSize: 10, color: "#b0c4ce", margin: 0 }}>
            Listed nearby
          </p>
        )}
      </div>
...
export default function NearbyDevices({ radius = 10 }) {
  ...
  const handleNext = useCallback(
    () => setIndex((i) => Math.min(products.length - VISIBLE, i + 1)),
    [products.length],
  );
          ...
          {showPrompt && (
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 180 }}>
                ...
                >
                  See refurbished phones, laptops and more being sold close to
                  you
                </p>
              </div>
              <button
                ...
                onMouseEnter={(e) => {
                  if (!locLoading) e.currentTarget.style.background = "#005f92";
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.background = "#0077b6";
                }}
