...
// ─── Main ─────────────────────────────────────────────────────────────────────
export default function ReviewSection({ productId, user, isAuthenticated, productOwnerId }) {
  ...
  const fetchReviews = useCallback(async (sort = sortBy) => {
    setLoading(true);
    try {
      const res  = await fetch(`${BASE}/products/${productId}/reviews?sortBy=${sort}&limit=50`);
  ...
  const handleSort = (s) => { setSortBy(s); fetchReviews(s); };
          ...
          <div className="bg-slate-50 rounded-2xl border border-slate-200 p-5 mb-8">
            ...
            {canAddMedia ? (
              <div className="mt-3 space-y-3">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {mediaPreviews.map((src, i) => (
                      <div key={i} className="relative w-16 h-16 rounded-xl overflow-hidden border border-slate-200 group">
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            setMediaImages(p => p.filter((_, idx) => idx !== i));
                    ...
                    {mediaImages.length < 5 && (
                      <label className="w-16 h-16 rounded-xl border-2 border-dashed border-slate-300 hover:border-[#1132d4]
                        ...
                        <input type="file" accept="image/jpeg,image/jpg,image/png,image/webp" multiple className="hidden"
                          onChange={e => {
                            const files = Array.from(e.target.files).slice(0, 5 - mediaImages.length);
                ...
                <div>
                  ...
                  {mediaVideo ? (
                    ...
                    <label className="flex items-center gap-2 border border-dashed border-slate-300 hover:border-[#1132d4]
                      ...
                      <input type="file" accept="video/mp4,video/mov,video/avi,video/mkv" className="hidden"
                        onChange={e => {
                          ...
                          if (f) { setMediaVideo(f); setMediaVideoName(f.name); }
                        }} />
                    </label>
                  )}
                </div>
              ...
              <p className="mt-3 text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2">
                📎 You've already attached media to one of your reviews on this product. Only one review per product can have photos/video.
              </p>
            )}
