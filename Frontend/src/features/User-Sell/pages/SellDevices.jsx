...
import { faShieldAlt, faBolt, faMoneyBillWave } from "@fortawesome/free-solid-svg-icons";
...
const benefits = [
  ...
  { icon: faBolt,          title: "Instant Payment",  description: "On accepting the price offered for your device, we'll arrange a free pick up." },
  { icon: faMoneyBillWave, title: "Best Price",        description: "Instant Cash will be handed over to you at time of pickup or through payment mode of your choice." },
];

export default function SellDevices({ models = [], brand, category, loading = false }) {
  ...
  const handleSelect = (model) => {
    if (!model?.id || !model?.name) { toast.error("Invalid model selected"); return; }
    setSelectedModel({ id: model.id, name: model.name, image: model.image, soldCount: model.soldCount });
  ...
  return (
    <>
      ...
      <section className="sd-header bg-gray-50 px-6 py-12">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="sd-search-wrap flex justify-end mb-6">
            <input
              value={query}
              onChange={(e) => { const v = e.target.value; if (v.length <= 50) setQuery(v); }}
              type="text"
          ...
          {loading ? (
            ...
            <p className="text-center text-gray-400 py-12">No models found for {brand}.</p>
          ) : (
            <div className="sd-grid grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6">
              {filtered.map((model) => (
                ...
                >
                  ...
                  <p className="text-sm font-medium text-gray-700 text-center">{model.name}</p>
                  {model.soldCount > 0 && (
                    <p className="text-xs text-teal-600 mt-1">{model.soldCount.toLocaleString("en-IN")}+ sold</p>
                  )}
                </div>
              ))}
            </div>
          ...
          {filtered.length === 0 && models.length > 0 && (
            <p className="text-center text-gray-400 py-6">No results found for "{query}"</p>
          )}
        </div>
      </section>
      ...
      <section className="sd-benefits bg-gray-50 px-6 py-12">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-3xl font-bold mb-10 text-center">Why Sell On Phonify?</h2>
          <div className="sd-benefits-grid grid grid-cols-1 md:grid-cols-3 gap-8">
            {benefits.map((b, i) => (
              <div key={i} className="flex flex-col items-center text-center bg-white rounded-lg shadow p-6 hover:shadow-md transition">
                <FontAwesomeIcon icon={b.icon} className="text-teal-600 text-5xl mb-4" />
