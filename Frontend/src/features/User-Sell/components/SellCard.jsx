    ...
    <>
      ...
      <section className="sell-body relative bg-[#f8feff] overflow-hidden">
        ...
        <div
          ...
          style={{ background: "radial-gradient(circle, rgba(0,180,216,0.18), transparent 70%)" }}
        />
        {/* Orb 2 */}
        <div
          className="orb-animate-delay absolute -bottom-16 left-[5%] w-[280px] h-[280px] rounded-full blur-[60px] pointer-events-none"
          style={{ background: "radial-gradient(circle, rgba(0,119,182,0.13), transparent 70%)" }}
        />
        ...
        <div className="sell-inner-grid relative max-w-7xl mx-auto px-6 md:px-16 py-16 grid md:grid-cols-2 gap-12 items-center">
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="search-wrap flex items-center gap-2 pl-4 pr-1.5 py-1.5 bg-white border-[1.5px] border-[rgba(0,180,216,0.25)] rounded-[18px] shadow-[0_4px_24px_rgba(0,119,182,0.08)] transition-all duration-200">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2.5" strokeLinecap="round" className="shrink-0">
              ...
              />
              <button
                onClick={() => { if (query.trim()) navigate(`/sell?search=${encodeURIComponent(query.trim())}`); }}
              ...
              {brandsLoading && (
                <div className="sell-brands-grid flex flex-wrap gap-2.5">
                  {Array.from({ length: VISIBLE_COUNT }).map((_, i) => (
                    <div key={i} className="shimmer w-20 h-[90px] rounded-2xl" />
                  ))}
                </div>
              )}

              {!brandsLoading && brandsError && (
                <p className="text-[13px] text-red-500 m-0">Could not load brands. Please try again.</p>
              )}
                ...
                <div className="sell-brands-grid flex flex-wrap gap-2.5">
                  {visibleBrands.map((brand) => {
                    ...
                    return logo ? (
                      ...
                      >
                        <img src={logo} alt={brand} className="w-[38px] h-[38px] object-contain" />
                        <span className="text-[11px] font-semibold text-[#03045e] text-center leading-tight">{brand}</span>
                      </button>
                    ) : (
                      <button
                        key={brand}
                  ...
                  {brands.length > VISIBLE_COUNT && (
                    ...
                    >
                      ...
                      <span>{showAll ? "Less" : `${brands.length - VISIBLE_COUNT} more`}</span>
                    </button>
                  )}
                </div>
              )}
          ...
          <div className="sell-right-panel relative hidden md:flex justify-center items-center">
            ...
            <div className="float-1 absolute top-[6%] -left-[4%] z-10 flex items-center gap-2.5 bg-white rounded-2xl px-3.5 py-2.5 shadow-[0_4px_20px_rgba(0,0,0,0.1)] whitespace-nowrap">
              <div className="w-8 h-8 rounded-[10px] flex items-center justify-center text-sm bg-[rgba(0,180,216,0.1)]">⚡</div>
              <div>
                <div className="text-[10px] font-medium text-[#94a3b8] leading-none mb-0.5">Avg. payout time</div>
                <div className="text-[13px] font-bold text-[#03045e]">2 Hours</div>
              </div>
            </div>
            ...
            <div className="float-2 absolute bottom-[8%] -right-[2%] z-10 flex items-center gap-2.5 bg-white rounded-2xl px-3.5 py-2.5 shadow-[0_4px_20px_rgba(0,0,0,0.1)] whitespace-nowrap">
              <div className="w-8 h-8 rounded-[10px] flex items-center justify-center text-sm bg-[rgba(3,4,94,0.07)]">📦</div>
              <div>
                <div className="text-[10px] font-medium text-[#94a3b8] leading-none mb-0.5">Orders completed</div>
                <div className="text-[13px] font-bold text-[#03045e]">1.2M+</div>
              </div>
            </div>
            ...
            <div className="phone-card">
              ...
              />
              <div style={{ display: "none", width: 160, height: 200, alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 8 }}>
                <svg width="80" height="120" viewBox="0 0 80 120" fill="none">
                  <rect x="8" y="2" width="64" height="116" rx="14" fill="#e8f4f8" stroke="#00b4d8" strokeWidth="2" />
                  <rect x="18" y="14" width="44" height="72" rx="4" fill="white" />
              ...
              <div className="w-full rounded-xl px-4 py-2.5 text-center bg-gradient-to-br from-[#03045e] to-[#0077b6] text-white">
                <div className="text-[10px] opacity-75 tracking-[0.05em] uppercase font-medium">Estimated value</div>
                <div className="sell-price-val text-[22px] font-extrabold tracking-[-0.01em]">Up to ₹18,000</div>
              </div>
            </div>
          </div>
        </div>
